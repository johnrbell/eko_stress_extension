// eko
let sID = '49603786932514'
document.addEventListener('DOMContentLoaded', function() {

  window.addEventListener('message', function(event) {
    if (event.origin !== 'https://video.eko.com') {
        return 
    }
    switch (event.data) {
      case 'show_modal':
      ...
      case 'variant_swap':
        ...
        break        
      default:
        console.log(`Error: ${event.data} is an unknown event.`)
    }      
  });
...
function showModal(){
  console.log('Eko_Event: showing modal')
  let modal = document.getElementById('sample_modal')
  modal.classList.toggle('active')  
}

function externalLink(){
  console.log('Eko_Event: launching external link')
  window.open('https://eko.com', '_blank');
}

function ekoATC(){
  console.log('Eko_Event: adding item to Shopify cart')
  
  let vID = document.querySelector('.product-variant-id').value
  let qty = document.querySelector('.quantity__input').value
...
function ekoCTAVarSwap(){
  if (sID == '49603786932514'){
    window.eko?.gallery?.setVariant('49603787194658');
    sID = `49603787194658`
  }else{
    window.eko?.gallery?.setVariant('49603786932514');
    sID = `49603786932514`
  }
...
function handleUrlChange() {
  ...
  if (window.location.pathname.includes('/products/')) {
    ...
    // Your product-specific code here
  }
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  productForms.forEach(form => {
    form.addEventListener('variant:changed', function(event) {
